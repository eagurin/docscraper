...
logger.add(
	...
	filter=lambda record: record["level"].name in ["INFO", "WARNING", "ERROR", "SUCCESS"]
)
logger.add(
...
class DocsiteToMD:
	def __init__(self, base_dir='docs_output', batch_size=10, rate_limit=5, max_concurrent=3):
	...
	async def initialize(self):
		...
		try:
			...
			markdown_worker_count = min(worker_count // 2, 10)  # Cap at 10 markdown workers
			...
			logger.info(f"Started {len(self.workers)} URL workers and {len(self.markdown_workers)} markdown workers")
		...
		for element in main_content.descendants:
			if element.name == 'pre':
				...
				if lang.startswith('language-'):
					...
					lang = 'json'
				elif any(keyword in code.lower() for keyword in ['function', 'class', 'def', 'return']):
					lang = 'python'
		...
		try:
			...
			try:
				...
				main_content = (
					...
					soup.find('body')  # Fallback to body if no other container found
				)
				...
				logger.debug(f"Extracted content - Title: {title}, Headers count: {len(headers)}, Content length: {len(content)}")
	...
	async def _process_batch(self, urls: List[tuple]):
		...
		try:
			...
			for i in range(0, len(urls), self.BATCH_SIZE):
				...
				tasks.extend([
					asyncio.create_task(self._process_single_url(url, domain, path_parts))
	...
	def _get_domain_limiter(self, domain: str) -> Semaphore:
		if domain not in self.domain_limiters:
			self.domain_limiters[domain] = Semaphore(5)  # 5 concurrent requests per domain
	...
	async def _update_progress(self):
		...
		if self.processed_count % 10 == 0:  # Log every 10 URLs
			logger.info(f"Progress: {self.processed_count}/{self.total_urls} URLs processed")
			if self.processed_count % self.gc_threshold == 0:
				import gc
				gc.collect()  # Force garbage collection periodically

	async def _process_single_url(self, url: str, allowed_domain: str, path_parts: List[str] = None):
		...
		self.visited_urls.add(url)  # Mark as visited before processing to prevent duplicates
			...
			async with domain_limiter, self.semaphore:  # Limit both domain and total concurrent requests
				...
				for link in links:
					if self.processing and link not in self.visited_urls:
						...
						logger.debug(f"URL already visited or processing stopped: {link}")

		except Exception as e:
			logger.error(f"Error processing {url}: {str(e)}", exc_info=True)


	async def _extract_links(self, content: str, base_url: str, allowed_domain: str) -> Set[str]:
				...
				try:
					...
					if doc_key not in processed_paths:
						...
						async with self.openai_semaphore:
							...
							await self.save_markdown(doc_data['title'], markdown_content, doc_data['path_parts'], doc_data['domain'])
					else:
						logger.debug(f"Skipping duplicate document: {doc_key}")
				except Exception as e:
					logger.error(f"Error processing markdown for {doc_data.get('title', 'unknown')}: {str(e)}")
			except Exception as e:
	...
	async def process_url(self, url: str, allowed_domain: str, path_parts: List[str] = None):
		...
		try:
			...
			await asyncio.gather(*self.workers, *self.markdown_workers, return_exceptions=True)
			self.workers = []
			self.markdown_workers = []
	...
	async def generate_combined_docs(self):
		...
		try:
			...
			for domain, docs in docs_by_domain.items():
				try:
					logger.debug(f"Generating combined documentation for domain: {domain}")
					...
					logger.error(f"Error generating combined docs for {domain}: {str(e)}")
					continue
...
async def main():
	...
	parser.add_argument('--wait-time', type=float, default=1.0, help='Wait time between requests')
